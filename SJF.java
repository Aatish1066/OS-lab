//This code assumes arrival time 0 can be modified easily for arrival time by adding a simple function of gapping and adjusting the formulas
//this adjustment and gapping function can be used from the STRF program
/*pid=0,1,2,3
...
public class SJF{
  ...
  //avg wait time
    ...
    private static void ascending_swap(int[] bt, int[] pid, int n) {
        for( int i=0;i<=n;i++){
            if(bt[i]>bt[i+1]){
                int tempbt,tempid;
                tempbt=bt[i+1];
